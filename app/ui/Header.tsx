...
export default function Header(){
    ...
    return (
        <header className={clsx(`${monaSans.className} fixed flex flex-row justify-center align-middle w-[100%] h-[15%] z-20`, sticky? `bg-mainwhite`: `bg-transparent`)}>
            <nav className="relative flex md:flex-row flex-col justify-center md:justify-end items-end md:items-center w-[100%] h-[100%]">
                ...
                <ul className={clsx(visible? `flex absolute shadow-md bg-white/50`: `hidden md:flex`, `md:relative top-[100%] flex-col justify-evenly items-center w-[100%] h-[120%] text-[1rem] md:text-[1rem] md:top-[0%] rounded-b-[20px] md:rounded-b-none p-5 list-none md:flex-row md:mr-10 md:w-[40%] md:h-[90%] md:shadow-none md:bg-transparent`)}>
                    <li className={clsx(`relative md:top-[8%] flex flex-col items-center justify-center w-[18%] h-[90%] text-center mb-2 after:content-[''] after:relative after:w-[90%] after:h-[2px] after:bg-mainred after:rounded-sm after:scale-x-0 after:transition-transform after:duration-300 after:ease-in hover:after:scale-x-100`, sticky && visible && 'text-black', !sticky && !visible && 'text-white')}><a href="/" target="_blank">Home</a></li>  
                ...
                <div className="flex justify-center items-center w-[60px] md:w-[40px] h-[60px] md:h-[40px] md:mr-20 mr-5 rounded-[100%] bg-mainred scale-100 transition-transform duration-100 ease-in hover:scale-110">
                    <Link className='flex justify-center items-center w-[60px] md:w-[40px] h-[60px] md:h-[40px]' href='/store'><CartIcon width={'60%'} height={'60%'} stroke="#ffffff"/></Link>
                </div>
            </nav>
        </header>
    )
